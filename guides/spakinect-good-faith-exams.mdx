...
<Warning>
Only SpaKinect admins can perform this step.
</Warning>
...
<Warning>
Only locations with a SpaKinect Location ID configured will be available when requesting GFEs.
</Warning>

## Setting Up Favorite Treatments

Once SpaKinect is enabled, you can mark certain spa treatments as favorites. This narrows down treatment lists across the application so your staff only sees the treatments your clinic actually offers, instead of the full SpaKinect catalog.

<Info>
The SpaKinect settings page is only visible if your tenant has the SpaKinect module enabled. If you don't see it, contact Decoda support to enable the module.
</Info>

### How to Set Favorites

1. Go to **Settings > SpaKinect**
2. You'll see the full list of available treatments from SpaKinect
3. Check the box next to each treatment your clinic offers
4. Click **Save Favorites**

**Tips for managing the list:**
- Use the **search bar** at the top to quickly find treatments by name
- Click **Select All** to check every treatment at once
- Click **Clear All** to uncheck everything and start fresh
- The counter at the top shows how many treatments you've selected (e.g., "5 of 120 treatments selected as favorites")

### How Favorites Affect Treatment Lists

- **When favorites are set** - Any time you select treatments (for example, when requesting a GFE), only your favorite treatments appear in the list. This keeps things simple and avoids scrolling through treatments you don't use.
- **When no favorites are set** - All available treatments from SpaKinect are shown. This is the default behavior if you haven't configured any favorites yet.

You can update your favorites at any time by returning to **Settings > SpaKinect** and making changes.
