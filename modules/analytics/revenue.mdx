...
The Product Sales Breakdown can show numbers on either a **Cash** or **Accrual** basis. Use the basis menu at the top of the dashboard to switch between them.

- **Cash** -- A sale counts on the day money is collected. A $500 package paid up front is $500 of revenue today, even if the patient hasn't used a single visit. This matches what your bank deposits look like.
- **Accrual** -- A sale counts on the day the service is delivered. The same $500 package only shows revenue as the patient redeems each visit. Memberships count revenue gradually across the billing period.

Most clinics use Cash for daily operations and Accrual when working with an accountant on financial statements. Click the **?** icon next to the basis menu for a side-by-side example.

To pick which basis the dashboard lands on for everyone, go to **Settings > General > Analytics** and set **Default Accounting Basis**. Each user can still change their own basis for the session.
