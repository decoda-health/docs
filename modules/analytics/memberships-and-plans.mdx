...
Four summary cards at the top give you a quick snapshot of your membership program:

| Card | What it shows |
|---|---|
| **Total Members** | Number of members across all statuses, with a trend indicator comparing to the previous month |
| **Projected MRR** | Monthly recurring revenue based on active memberships, with a trend indicator comparing to the previous month. Based on each plan's configured price -- processing fees passed to patients aren't counted here. |
| **New Members in [Month]** | Number of new memberships started this month, with a trend indicator comparing to the previous month |
| **Cancelled in [Month]** | Number of cancellations this month, with a trend indicator comparing to the previous month (for cancellations, an increase is shown in red) |

### Charts

Two bar charts appear above the table. These charts show all-time data and are not affected by the table filters:

- **Memberships by Status** -- Shows each membership plan as a stacked bar, colored by status (Active, Frozen, Cancelled, Delinquent, or Expired). A quick glance tells you the overall health of each plan.
...
| **Price** | The recurring price they're being charged. If the plan passes the credit card surcharge to the patient, that fee appears as a separate **Processing Fee** line on the receipt, not in this column. |
| **Start Date** | When the membership started |
| **Created Date** | When the membership record was created (the table defaults to showing the last 90 days by created date) |
...
| **Sold By** | Filter by the provider who enrolled the member |
| **Created Date** | Filter by a date range for when memberships were created |
...
| **Start Date** | Memberships grouped by start date |
| **Created Date** | Memberships grouped by when they were created |
...
| **Created** | When the plan was created (hidden by default) |
| **Location** | Clinic location (hidden by default) |
| **Payment Method** | Card or method used for automatic charges (hidden by default) |
