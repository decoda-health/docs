...
<Steps>
  ...
  <Step title="Create Service">
    Click the **Create Service** button in the top right corner. A drawer will open with a tabbed editor.
  </Step>
...
- **Service Timeline:** A visual editor that lets you configure the full appointment flow:
  ...
  - **Cleanup Time:** Optional post-appointment block (for example, room turnover) with its own duration, custom name, and calendar color.
  - **Cleanup Providers:** Assign specific staff members qualified to handle cleanup work (separate from the main service provider).
...
Lets patients pick optional related services to add to this booking. For example, a "Botox" service can offer "Lip Filler" and "Microneedling" as add-ons that the patient can attach during self-scheduling.

- **Minimum Selections:** The fewest add-ons a patient must choose. Leave empty to make selections optional.
- **Maximum Selections:** The most add-ons a patient can choose. Leave empty for unlimited.
- **Add-On Services:** Click **Add Service** to link other services as selectable add-ons. Drag to reorder how they appear to patients.
- **Default:** Toggle on an add-on to have it pre-selected when the patient sees the booking form.
- **Add-On Only:** Toggle on to hide that service from normal self-scheduling so it can only be booked as an add-on to another service.

A live preview at the bottom of the tab shows what patients will see during booking.
...
- **Reminder Before Appointment:** Toggle on to send a reminder SMS. Configure how far in advance (in minutes, hours, or days) the reminder is sent, and customize the message template. The default reminder asks patients to text **1** to confirm or **2** to cancel their appointment — edit the template if your clinic doesn't accept text confirmations.
...
Build a multi-step automation that runs around each appointment for this service. Use sequences when you need more than a single reminder or follow-up — for example, a booking confirmation, a 24-hour reminder, a same-day check-in, and a next-day review request, all from one place.

- **Active toggle:** Turn the sequence on or off for this service. When off, no steps run.
- **Trigger tabs:** Steps are grouped by what kicks them off:
  - **Appointment Booked** — when a new appointment is created.
  - **Appointment Starting** — before or after the appointment start time.
  - **Appointment Completed** — after the appointment is marked complete.
  - **Appointment Cancelled** — when the appointment is cancelled.
  - **Appointment No-Show** — when a patient is marked as a no-show.
- **Add Step:** Click **Add Step** inside a trigger tab to create a new step. Each step has:
  - **Action:** Send SMS, Send Email, or Change Status. (Change Status is not available for cancelled or no-show triggers.)
  - **Timing:** How long before or after the trigger to run the step. Only the **Appointment Starting** trigger supports running before the event; the others run after.
  - **Message content:** SMS template, or email subject and body. Both support variables and AI-generated drafts. Email steps also support attachments.
  - **Target Status:** For Change Status steps, the appointment status to set.
  - **Condition (optional):** Skip the step unless a condition is met — for example, only send the message if the patient has not completed their intake form, or has not confirmed the appointment.

The **All** tab shows every step across every trigger in one timeline so you can see the full flow.

<Info>
  This tab is only visible if your clinic has the Sequences module enabled.
</Info>
...
2. A bulk edit drawer opens with tabs for **Pricing**, **Scheduling**, **Providers & Locations**, **Forms**, **Notifications**, **Sequences** (if the Sequences module is enabled), **Visibility**, **Checklists**, and **Settings**.
...
<AccordionGroup>
  ...
  <Accordion title="Permissions">
    - **Manage Services** -- Required to create, edit, bulk edit, and archive services. Without this permission, users can view services but cannot make changes.
