...
<Steps>
  ...
  <Step title="Configure the Assistant">
    ...
    - **Tools:** Select capabilities the assistant can use during calls (e.g., ending calls, sending texts, transferring calls, verifying callers). Scheduling tools are also available -- these let the assistant find open appointment times, book appointments, cancel appointments, and reschedule appointments on behalf of callers. Note that some tools depend on others: for example, enabling the booking tool will automatically enable the availability lookup tool, since the assistant needs to find open times before it can book.
...
When editing an assistant's behavior, you will see a row of preset buttons above the behavior text area (labeled **Quick Presets**). Each preset covers a specific capability -- for example, rules for recommending related services during booking, or steps for reducing cancellations by offering alternatives first.

**How to use presets:**

1. Open the assistant editor and scroll to the **Assistant Behaviour** field.
2. Click the **+** button on any preset card to insert its instructions into the behavior text area.
