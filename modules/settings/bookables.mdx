...
| **Capacity** | The maximum number of appointments that can use this resource at the same time |
...
<Steps>
  ...
  <Step title="Set Capacity">
    Enter the maximum number of appointments that can use this resource at the same time. The default is 1, which means only one appointment can use the resource per time slot. Increase this if the resource can be shared (for example, a waiting room that fits multiple patients).
  </Step>
...
1. Click the **Capacity** cell in the table row you want to change.
2. A small popup appears with a number input. Use the arrows or type a new value (minimum 1).
3. Press **Enter** or click **Save** to confirm. Press **Escape** or click **Cancel** to discard changes.
