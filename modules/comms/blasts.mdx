...
<Steps>
  ...
  <Step title="Compose Your Message">
    Type your message in the text editor. As you type, the character count and segment count update below the editor so you can keep messages concise.

    You can personalize each message using template variables. Type **@** in the editor to see available variables like `{first_name}`, `{last_name}`, and `{tenant_name}`. Each patient receives a customized version of your message with their information filled in.
...
<Steps>
  ...
  <Step title="Compose Your Email">
    Use the rich text editor to write your email. The editor supports headings, bold/italic text, images, buttons, links, blockquotes, and lists. Type **/** to see available block types you can insert.

    To personalize the email, type **@** to insert template variables like `{first_name}` or `{tenant_name}`. Each recipient gets a customized version.

    Switch to **Code** mode if you prefer to write raw HTML directly.
...
Template variables let you personalize each message with patient-specific information. The system replaces the variable with the actual value for each recipient.
...
| `{preferred_name}` | Patient's preferred name (falls back to first name) |
| `{date_of_birth}` | Patient's date of birth (e.g., January 15, 1990) |
| `{birthday}` | Patient's birthday without the year (e.g., January 15) |
| `{phone_number}` | Patient's phone number |
| `{email}` | Patient's email address |
| `{patient_id}` | Patient's ID |
| `{tenant_name}` | Your organization's name |

Type **@** in the editor to see and insert available variables.
...
- **Sent** -- Emails accepted by the mail server.
...
- **SMS:** Cost depends on message length (number of segments) and whether images are attached (MMS). The estimate updates in real-time as you type.
