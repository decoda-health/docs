...
- **Messages** -- Automated SMS or email messages attached to this appointment, each with its own timing (e.g., "1 day before", "3 days after"). See [Messages on an Appointment](#messages-on-an-appointment).
...
A message step sends an automated SMS to the patient on its own scheduled date. Message steps appear on the timeline alongside appointments and tasks and are sent when their scheduled date arrives.

Use a message step for general check-ins or instructions that aren't tied to a specific visit. For reminders, confirmations, or follow-ups around a particular appointment, attach messages to the appointment step instead — see [Messages on an Appointment](#messages-on-an-appointment).

When you click on a message step, the details area shows:

- **Message Body** -- The SMS text. You can personalize the message with variables by typing `@` in the editor. Available variables include:
...
To add one, click **Add Step** and choose **Message**, then write the body.

### Messages on an Appointment

Each appointment step can carry its own list of automated messages that fire relative to the appointment time — for example, a reminder one day before, a confirmation at the scheduled time, and a follow-up three days after. These messages live on the appointment; they don't appear as separate steps on the timeline. Each appointment card shows a badge with the count (e.g., "3 messages").

To add or edit them, open the appointment step and scroll to the messages section at the bottom. Click **Add a Sequence** to create the first message; once any exist, click **View N Messages** to open the list, then **Add Message** to add another or click an existing message to edit. For each message:

- **Effect** — choose **Send SMS** or **Send Email** (email messages also take a **Subject**).
- **Timing** — set how far before or after the appointment time it fires (e.g., "2 days before"), or turn on **Immediately** to fire at the scheduled time. A preview sentence shows the chosen timing.
- **Email body** or **Message body** (the label depends on the **Effect**) — write the content. Type `@` to insert personalization variables.

Click **Back** to return to the appointment details. Messages save with the rest of the care plan. To remove a message, open it and click **Remove Message**.

When a care plan is created from a template, the template's attached messages are copied over and can be edited on the patient's plan.

### Decision Steps

A decision step is a point in the care plan where the treatment path splits into two or more options, called **branches**. Each branch contains its own list of appointment, task, message, and (optionally) further decision steps. Only one branch can be selected for a given patient — the others are skipped.

Branches can contain their own decision steps, letting you build multi-level paths (for example, a first decision picks the treatment type, and a nested decision inside the chosen branch picks the dosage). When a branch is skipped, everything inside it — including nested decisions and their branches — is skipped automatically.
...
<Steps>
  ...
  <Step title="Add Steps to Each Branch">
    Click into a branch and add appointment, task, message, or decision steps. Configure services, time gaps, and other details the same way as steps on the main timeline.
  </Step>
  <Step title="Add More Branches (Optional)">
    Click **Add option** to create additional paths if needed.
  </Step>
...
You can also pick a branch by scheduling its first appointment. When the decision step is up next on the active path, the **Schedule Appointment** button appears on the first appointment of every branch. Booking it selects that branch, marks the decision step as Completed, and skips the other branches in one step — there's no separate confirmation prompt.

<Warning>
  Like clicking **Select this path**, scheduling the first appointment of a branch commits to that branch and cannot be undone.
</Warning>
...
- **Add or remove branches** -- click **Add option** or remove a branch from the decision step details.
...
When an appointment step is Pending and has at least one service selected, a **Schedule Appointment** button appears in the step details. This lets you create a calendar appointment directly from the care plan. If the next thing on the active path is a decision step, the button also appears on the first appointment of every branch — see [Auto-Select by Scheduling the First Step](#auto-select-by-scheduling-the-first-step).
