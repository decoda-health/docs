...
- **Messages** -- Automated SMS or email messages (called **sequences**) attached to this appointment, each with its own timing (e.g., "1 day before", "3 days after"). See [Appointment Message Sequences](#appointment-message-sequences).
...
Use a message step for general check-ins or instructions that aren't tied to a specific visit. For reminders, confirmations, or follow-ups around a particular appointment, attach a sequence to the appointment step instead — see [Appointment Message Sequences](#appointment-message-sequences).
...
Open the appointment step and scroll to the messages section at the bottom.

- If the appointment has no sequences yet, click **Add a Sequence**. The editor opens directly.
...
Click **Back** to return to the appointment details. Sequences save with the rest of the care plan. To remove one, open it and click **Remove Message**.

When a care plan is created from a template, the template's attached sequences are copied over and can be edited on the patient's plan.

#### Copy a Sequence to Other Steps

If an appointment step's sequences should also apply to other appointment steps in the plan, you can copy them across in one go. Open the source step, find the **Sequences** section, and click **Copy**. A banner appears at the top of the editor — click each appointment step you want to copy the sequences onto, then click **Apply** to add the same set of messages to every selected step. Click **Cancel** to back out without changing anything.

<Warning>
  **Apply** replaces the existing sequences on each target step with a copy of the source step's sequences. There is no undo.
</Warning>

#### Multi-Service Appointments

If an appointment step has more than one service, automated sequences configured at the service level (**Settings > Services > Sequences**) only fire for the appointment's **primary** service. Sequences tied to the other services on the same appointment are skipped. Sequences attached directly to the step are not affected.
