...
| **Message** | An automated SMS to the patient — either linked to an appointment or sent on its own | Message type (optional), SMS body, timing (if linked to an appointment) |
...
A message step sends an automated SMS to the patient. There are two ways to use message steps:

- **Linked messages** — tied to an appointment step. You choose a message type (reminder, confirmation, or follow-up) and set the timing relative to that appointment. Linked messages appear on the timeline next to the appointment they belong to.
- **Standalone messages** — not tied to any appointment. These appear as their own step on the timeline, just like an appointment or task step. No message type is needed — just write the SMS body. Standalone messages are useful for general check-ins, instructions, or any message that is not related to a specific visit.

When a message is linked to an appointment, there are three types to choose from:
...
<Info>
  Message type is only required when the message is linked to an appointment. Standalone messages do not need a type — just the SMS body.
</Info>

When you click on a message step, the details area shows:

- **Message Type** -- The type of message (reminder, confirmation, or follow-up). This field only applies to messages linked to an appointment and can be left blank for standalone messages.
- **Timing** -- How far before or after the appointment the message is sent. This only appears when the message is linked to an appointment (not shown for confirmation messages or standalone messages).
...
<Steps>
  <Step title="Add a Message Step">
    Click **Add Step**, then choose **Message** from the menu. The message is added as its own step on the timeline.
  </Step>
  <Step title="Choose a Message Type (Optional)">
    If this message is linked to an appointment, select whether it is a **Reminder**, **Confirmation**, or **Follow-up**. For standalone messages that are not tied to an appointment, you can skip this — no message type is needed.
  </Step>
  <Step title="Set the Timing (Linked Messages Only)">
    For reminder and follow-up messages linked to an appointment, set how far before or after the appointment the message should be sent. This step does not apply to standalone messages.
  </Step>
...
<Info>
  Message steps can work in two ways: **linked** to an appointment (with a type and timing) or **standalone** (sent on their own, without an appointment). Linked messages appear on the timeline next to the appointment they belong to. Standalone messages appear as their own step, just like appointments and tasks.
</Info>
...
| **Message** | Message type (optional), SMS body, timing (if linked to an appointment) |
